...
// --- OS-SPECIFIC TIMING (From previous version) ---
#ifdef _WIN32
    #include <windows.h>
...
void add_round_key(uint8_t state[16], const uint8_t roundKey[16]) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= roundKey[i];
    }
}
...
int main() {
    ...
    uint8_t state[16];
    uint8_t roundKeys[11][16] = {0}; 

    timer_t start, end;
    printf("Starting 1D-Flattened Benchmark (%d iterations)...\n", ITERATIONS);
