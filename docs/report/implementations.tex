...
A third optimization targeted the mix\_columns and inv\_mix\_columns transformations. Instead of using a general purpose multiplication routine in the finite field $GF(2^8)$, the optimized implementation replaces these operations with fixed sequences of operations tailored to the specific coefficients required by AES. In particular, multiplication by 0x02 is implemented using the xtime operation, and multiplication by other constants is constructed from combinations of this primitive and XOR operations based on the tables in \ref{fig:mix_columns_tables}. The new state is calculated from the multiplication of values from the old state and values from the tables in \ref{fig:mix_columns_tables}.
...
These changes transform the implementation from a readable version into a performance oriented version that reduces overhead, and increases instruction level parallelism. These optimizations improve the overall efficiency for encryption by around 50\%.
